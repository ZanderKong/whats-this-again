...
test("migrates schema 1 to 2 while preserving settings and memories", async () => {
  ...
  assert.equal(area.data.inlineai_schema_version, 2);
...
test("preserves existing annotations and is idempotent", async () => {
  const C = loadConstants();
  const initial = { inlineai_schema_version: 2, inlineai_memories: {}, inlineai_annotation_batches: { b1: { id: "b1" } }, inlineai_active_annotation_batches: { page: "b1" } };
